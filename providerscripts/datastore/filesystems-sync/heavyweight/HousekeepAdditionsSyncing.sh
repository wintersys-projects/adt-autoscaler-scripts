...
do
        if ( [ "`${HOME}/providerscripts/datastore/operations/AgeOfDatastoreFile.sh "${bucket_type}" "filesystem-sync/${bucket_type}/additions/${addition}" "${target_directory}"`" -gt "60" ] )
        then
                ${HOME}/providerscripts/datastore/operations/DeleteFromDatastore.sh "${bucket_type}" "filesystem-sync/${bucket_type}/additions/${addition}" "distributed" "${target_directory}"
        fi
done
