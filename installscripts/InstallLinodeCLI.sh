...
then
    ...
    then
        ...
        linodecli="`/usr/bin/find ~/.local/pipx -name "linode-cli" -print | /usr/bin/head -1`/bin/linode-cli"
        /bin/rm -r /usr/local/bin/linode-cli
        /usr/bin/ln -s ${linodecli} /usr/local/bin/linode-cli
    fi
fi
